...
RUN git clone https://github.com/DaeGyeong/Python.git
WORKDIR /apps/web_root/Python
...
RUN echo "WSGIScriptAlias / /apps/web_root/Python/learning_log/wsgi.py" >> /apps/apache2/conf/httpd.conf
RUN echo "WSGIPythonPath /apps/web_root/Python" >> /apps/apache2/conf/httpd.conf
RUN echo "WSGIDaemonProcess testPython lang='ko_KR.UTF-8' locale='ko_KR.UTF-8'" >> /apps/apache2/conf/httpd.conf
RUN echo "<Directory /apps/web_root/Python/learning_log>" >> /apps/apache2/conf/httpd.conf
...
RUN ./bin/mysql -u root -proot -e "use mysql; set password=password('root');"
